export const app = {
   mySocial: {
      telegram: 'https://t.me/nazar_pryt',
      gitHub: 'https://github.com/nazarPryt',
      linkedin: 'https://linkedin.com/in/nazar-prytuliak',
      whatsapp:
         'https://api.whatsapp.com/send?phone=393925224866&text=Hi%20Nazar!%20Saw%20your%20portfolio%20%E2%80%94%20let%E2%80%99s%20discuss!%F0%9F%91%8D',
   },
   projects: {
      uiKit: {
         npm: 'https://www.npmjs.com/package/@nazar-pryt/inctagram-ui-kit',
         github: 'https://github.com/nazarPryt/inctagram-ui-kit',
         frontend: 'https://inctagram-ui-kit.vercel.app',
      },
      InstaAdmin: {
         github: 'https://github.com/nazarPryt/inctagram-admin',
         frontend: 'https://insta-admin.nazarit.com',
      },
      ShopV1: {
         github: 'https://github.com/nazarPryt/Online-Store',
         frontend: 'https://shop.nazarit.com',
         backend: 'https://github.com/nazarPryt/back-for-shop-v3',
      },
      Instareplica: {
         github: 'https://github.com/nazarPryt/inctagram',
         frontend: 'https://shotshare.nazarit.com',
      },
      QuizCards: {
         github: 'https://github.com/KauhareniaNastassia/Cards',
         frontend: 'https://kauharenianastassia.github.io/Cards/#/login',
      },
   },
} as const
